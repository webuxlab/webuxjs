...
const reserved = ['error', 'connect', 'disconnect', 'disconnecting', 'newListener', 'removeListener', 'ping', 'pong'];
...
function SocketOn(socket, name, fn, log) {
  if (name.includes('_ReservedEvents') && reserved.includes(name.split('_')[0]) && name.split('_')[0] === 'connect') {
    log.debug(`webux-socket - 'SocketOn' - Adding 'onConnect' function "${name.split('_')[0]}" to the socket`);
  } else if (name.includes('_ReservedEvents') && reserved.includes(name.split('_')[0])) {
    log.debug(`webux-socket - 'SocketOn' - Attaching 'reserved event' "${name.split('_')[0]}" to the socket`);
 ...
 */
function AttachAction(_path, socket, io, log, recursionAllowed, ignoreFirstDirectory, parent = '') {
  if (fs.existsSync(path.join(_path)) && fs.lstatSync(path.join(_path)).isDirectory()) {
    log.debug(`webux-socket - 'AttachAction' - Get actions for this path "${_path}"`);
    ...
    if (!ignoreFirstDirectory) {
      parentFolder = FirstLetterCaps(parent) + FirstLetterCaps(_path.split(path.sep)[_path.split(path.sep).length - 1]);

      log.debug(`webux-socket - 'AttachAction' - Current Parent Folder "${parentFolder}"`);
    }

    log.debug(`webux-socket - 'AttachAction' - Get all actions within this path "${actions}"`);
    actions.forEach((_method) => {
      if (!_method.includes('.js') || !require(path.resolve(path.join(_path, _method))).socket) {
        if (fs.existsSync(path.join(_path, _method)) && fs.lstatSync(path.join(_path, _method)).isDirectory() && recursionAllowed) {
          // call the function again if this is a directory
          return AttachAction(path.join(_path, _method), socket, io, log, recursionAllowed, false, parentFolder);
        }
        log.debug(`webux-socket - 'AttachAction' - Invalid Method ${_path}/${_method}`);
        return _method;
      }
      log.debug(`webux-socket - 'AttachAction' - Attaching the method "${_method}"`);
      const name = _method.split('.js')[0] + parentFolder;

      log.debug(`webux-socket - 'AttachAction' - Method name "${name}"`);

      return SocketOn(socket, name, require(path.resolve(path.join(_path, _method))).socket(socket, io), log);
    });
  } else if (
    ...
    const parentFolder = FirstLetterCaps(_path.split(path.sep)[_path.split(path.sep).length - 2]);
    log.debug(`webux-socket - 'AttachAction' - Current Parent Folder "${parentFolder}"`);

    const method = _path.split(path.sep)[_path.split(path.sep).length - 1];

    log.debug(`webux-socket - 'AttachAction' - Attaching the method "${method}"`);
    const name = method.split('.js')[0] + parentFolder;

    log.debug(`webux-socket - 'AttachAction' - Method name "${name}"`);

    log.debug(`webux-socket - 'AttachAction' - Attaching the method "${method}" with name "${name}" to the socket`);
    return SocketOn(socket, name, require(path.resolve(path.join(_path))).socket(socket, io), log);
  } else {
...
function AttachActions(log, config) {
  return (socket, io, paths) => {
    log.debug(`webux-socket - 'AttachAction' - List of paths to be iterated "${paths}"`);
    paths.forEach((_path) => AttachAction(_path, socket, io, log, config.recursionAllowed, config.ignoreFirstDirectory));
  };
}
...
export default function Actions() {
  ...
  Object.keys(this.config.namespaces).forEach((namespace) => {
    ...
    if (namespace === 'default') {
      this.log.debug("webux-socket - 'LoadActions' - Adding the 'default' namespace");
      this.io.on('connection', (socket) => {
        this.log.debug("webux-socket - 'LoadActions' - Attaching the actions to the 'default' namespace");
        AttachActions(this.log, this.config)(socket, this.io, paths);
        this.log.debug("webux-socket - 'LoadActions' - Actions attached the 'default' namespace");
      });
    } else {
      this.log.debug(`webux-socket - 'LoadActions' - Adding the '${namespace}' namespace`);
      this.io.of(`/${namespace}`).on('connection', (socket) => {
        this.log.debug(`webux-socket - 'LoadActions' - Attaching the actions to the '/${namespace}' namespace`);
        AttachActions(this.log, this.config)(
          socket, // socket
          this.io.of(`/${namespace}`), // io
          paths,
        );
        this.log.debug(`webux-socket - 'LoadActions' - Actions attached the '/${namespace}' namespace`);
      });
    }
  });
}
