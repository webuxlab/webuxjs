...
try {
  ...
  prompt(questions).then((answers) => {
    try {
      ...
      const files = [
        path.join(backendDir, 'api', apiVersion, 'actions', resourceFolder, resourceFilename),
        path.join(backendDir, 'tests', 'cases', testFilename),
        path.join(backendDir, 'api', apiVersion, 'validations', resourceFilename),
      ...
      const options = {
        ...
        to: [resourceName, author, license, creationDate, modelName, resourceFilename, plurial],
      };
      ...
      Promise.all([processFiles(files, resourceFolder)])
        .then(() => {
          ...
          updateInfo(options)
            .then(async () => {
              try {
                await updateRoute(backendDir, resourceName, resourceFolder, method, apiVersion);
              } catch (error) {
