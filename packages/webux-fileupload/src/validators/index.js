 ...
 */
export const UploadFile = async (options, files, filename, label = '', log = console) => {
  try {
    // req.files.KEY
    ...
    if (!localFiles || localFiles.length === 0) {
      throw new Error(`No files provided using ${options.express.key} for the key.`);
    }
    ...
    for (const file of fileToProcess) {
      ...
      if (options.mimeTypes.includes(file.mimetype)) {
        ...
        await moveFile(file, uploadTempDestination, log);
    ...
    if ((!fileProcessed || fileProcessed.length === 0) && (!fileErrored || fileErrored.length === 0)) {
      throw new Error('An error has occured while processing uploaded files.');
    }

    return { fileProcessed, fileErrored };
  } catch (e) {
    log.error(e);
    throw new Error(
